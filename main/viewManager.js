...
function createView (existingViewId, id, webPreferences, boundsString, events) {
  ...
  view.webContents.setWindowOpenHandler(function (details) {
    ...
    /*
      ...
      when it is clicked.
      (https://github.com/trustbrowserbrowser/trustbrowser/issues/1835)
    */
  ...
  view.webContents.on('ipc-message', function (e, channel, data) {
    ...
    try {
      ...
      // https://github.com/trustbrowserbrowser/trustbrowser/issues/2052
  ...
  function handleExternalProtocol (e, url, isInPlace, isMainFrame, frameProcessId, frameRoutingId) {
    var knownProtocols = ['http', 'https', 'file', 'trustbrowser', 'about', 'data', 'javascript', 'chrome'] // TODO anything else?
  ...
  */
  view.webContents.on('did-start-navigation', function (event) {
    if (event.isMainFrame && !event.isSameDocument) {
      ...
      const shouldHaveJS = (!(settings.get('filtering')?.contentTypes?.includes('script'))) || event.url.startsWith('trustbrowser://')
...
function setView (id, senderContents) {
  ...
  // see https://github.com/trustbrowserbrowser/trustbrowser/issues/1966
...
function focusView (id) {
  // empty views can't be focused because they won't propogate keyboard events correctly, see https://github.com/trustbrowserbrowser/trustbrowser/issues/616
