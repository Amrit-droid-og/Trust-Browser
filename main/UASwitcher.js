...
see https://github.com/trustbrowserbrowser/trustbrowser/issues/657 for more information */
...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/Trust Browser\/\S+\s/, '').replace(/Electron\/\S+\s/, '').replace(process.versions.chrome, process.versions.chrome.split('.').map((v, idx) => (idx === 0) ? v : '0').join('.'))
}
...
see https://github.com/trustbrowserbrowser/trustbrowser/issues/868
*/
