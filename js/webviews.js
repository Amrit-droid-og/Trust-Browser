...
function onPageURLChange (tab, url) {
  if (url.indexOf('https://') === 0 || url.indexOf('about:') === 0 || url.indexOf('chrome:') === 0 || url.indexOf('file://') === 0 || url.indexOf('trustbrowser://') === 0) {
    tabs.update(tab, {
      secure: true,
      url: url
    })
...
const webviews = {
  ...
  asyncCallbacks: {},
  internalPages: {
    error: 'trustbrowser://app/pages/error/index.html'
  },
  events: [],
  IPCEvents: [],
  ...
  emitEvent: function (event, tabId, args) {
    if (!webviews.hasViewForTab(tabId)) {
      // the view could have been destroyed between when the event was occured and when it was recieved in the UI process, see https://github.com/trustbrowserbrowser/trustbrowser/issues/604#issuecomment-419653437
      return
    }
  ...
  add: function (tabId, existingViewId) {
    ...
    if (!existingViewId) {
      if (tabData.url) {
        ...
        // workaround for https://github.com/trustbrowserbrowser/trustbrowser/issues/872
        ipc.send('loadURLInView', { id: tabData.id, url: urlParser.parse('trustbrowser://newtab') })
      }
    }
...
settings.listen(function () {
  tasks.forEach(function (task) {
    task.tabs.forEach(function (tab) {
      if (tab.url.startsWith('trustbrowser://')) {
        try {
...
webviews.bindIPC('downloadFile', function (tabId, args) {
  if (tabs.get(tabId).url.startsWith('trustbrowser://')) {
