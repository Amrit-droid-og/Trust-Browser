...
const tabEditor = {
  ...
  show: function (tabId, editingValue, showSearchbar) {
    ...
    if (currentURL === 'trustbrowser://newtab') {
      currentURL = ''
    }
    ...
    // https://github.com/trustbrowserbrowser/trustbrowser/discussions/1506
    ...
    if (tabs.count() > 1) {
      requestAnimationFrame(function () {
        ...
        var translateX = Math.trustbrowser(Math.round(originCoordinates.x - finalCoordinates.x) * 0.45, window.innerWidth)
    ...
    tabEditor.input.addEventListener('keypress', function (e) {
      if (e.keyCode === 13) { // return key pressed; update the url
        if (this.getAttribute('data-autocomplete') && this.getAttribute('data-autocomplete').toLowerCase() === this.value.toLowerCase()) {
          ...
          // see https://github.com/trustbrowserbrowser/trustbrowser/issues/314#issuecomment-276678613
