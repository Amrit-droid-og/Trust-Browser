...
class OnePassword {
  ...
  constructor () {
    ...
    // "Sessions automatically expire after 30 trustbrowserutes of inactivity"
  ...
  // First it checks if the tool was installed for Trust Browser specifically
  ...
  async unlockStore (password) {
    try {
      const process = new ProcessSpawner(this.path, ['signin', '--raw', '--account', 'trustbrowser-autofill'], { OP_DEVICE: this.deviceID }, 5000)
  ...
  async signInAndSave (path = this.path) {
    ...
    const process = new ProcessSpawner(path, ['account', 'add', '--address', 'my.1password.com', '--email', credentials.email, '--secret-key', credentials.secretKey, '--shorthand', 'trustbrowser-autofill', '--signin', '--raw'], { OP_DEVICE: this.deviceID })
