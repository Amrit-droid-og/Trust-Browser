...
if (process.isMainFrame) {
  ...
  setTimeout(function () {
    ...
    electron.webFrame.executeJavaScript(`
      history.pushState = ( f => function pushState(){
        ...
        window.postMessage('_trustbrowserInternalLocationChange', '*')
        return ret;
    })(history.pushState);
    
    history.replaceState = ( f => function replaceState(){
        var ret = f.apply(this, arguments);
        window.postMessage('_trustbrowserInternalLocationReplacement', '*')
        return ret;
  ...
  window.addEventListener('message', function (e) {
    if (e.data === '_trustbrowserInternalLocationChange') {
      setTimeout(function () {
        getPageData(function (data) {
          ipc.send('pageData', data)
        })
