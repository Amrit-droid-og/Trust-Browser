    ...
    const sortable = new Sortable(el, {
      ...
      onEnd: function (e) {
        ...
        sortedItems.forEach(function (item) {
          var tabId = item.getAttribute('data-tab')
          var previousTask = tasks.getTaskContainingTab(tabId) // note: can't use e.from here, because it contains only a single element and items could be cotrustbrowserg from multiple tasks
  ...
  initialize: function () {
    ...
    document.getElementById('navbar').addEventListener('wheel', function (e) {
      if (e.altKey || e.ctrlKey || e.metaKey || e.shiftKey) {
        // https://github.com/trustbrowserbrowser/trustbrowser/issues/698
        return
      }
