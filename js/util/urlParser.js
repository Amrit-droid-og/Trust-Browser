...
var urlParser = {
  ...
  parse: function (url) {
    ...
    if (url.startsWith('trustbrowser:') && !url.startsWith('trustbrowser://app/')) {
      // convert shortened trustbrowser:// urls to full ones
      const urlChunks = url.split('?')[0].replace(/trustbrowser:(\/\/)?/g, '').split('/')
      const query = url.split('?')[1]
      return 'trustbrowser://app/pages/' + urlChunks[0] + (urlChunks[1] ? urlChunks.slice(1).join('/') : '/index.html') + (query ? '?' + query : '')
    }
  ...
  isInternalURL: function (url) {
    return url.startsWith('trustbrowser://')
  },
    ...
    if (urlParser.isInternalURL(url)) {
      ...
      if (representedURL) {
        ...
        try {
          ...
          var urlObj = new URL(url)
          if (pageName) {
            return 'trustbrowser://' + pageName[1] + urlObj.search
          }
        } catch (e) {}
      }
    }
