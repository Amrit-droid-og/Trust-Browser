...
* `trustbrowserContentLength` (number, default `140`): the minimum node content length used to decide if the document is readerable;
* `trustbrowserScore` (number, default `20`): the minimum cumulated 'score' used to detertrustbrowsere if the document is readerable;
* `visibilityChecker` (function, default `isNodeVisible`): the function used to detertrustbrowsere if a node is visible;
