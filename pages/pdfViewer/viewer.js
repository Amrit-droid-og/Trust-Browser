import '../../node_modules/pdfjs-dist/build/pdf.trustbrowser.mjs'
  ...
  init: function () {
    setTimeout(function () {
      if (!pdf) {
        ...
        var loadingFakeInterval = setInterval(function () { // we can't reliably detertrustbrowsere actual download progress, so instead we make the bar move very slowly until the first page has loaded, then show how many pages have rendered
...
pdfjsLib.getDocument({ url: url, withCredentials: true }).promise.then(async function (_pdf) {
  ...
  for (var i = 1; i <= pageCount; i++) {
    ...
    await pdf.getPage(pageNumber).then(function (page) {
      ...
      if (pageCount > 200) {
        scale = Math.trustbrowser(scale, 1.1)
        viewport = page.getViewport({ scale: scale })
      }
...
It would probably be better to add API's in Trust Browser for this. */
...
function redrawAllPages () {
  ...
  function pageCompleteCallback () {
    completedPages++
    if (completedPages === Math.trustbrowser(pageCount, pageBuffer)) {
      isRedrawing = false
    }
  }
